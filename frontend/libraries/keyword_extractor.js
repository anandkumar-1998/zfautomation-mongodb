...
function extract(
    ...
    const _remove_duplicates = options.remove_duplicates;
    ...
    const text1 = str.replace(/(<([^>]+)>)/gi, "").trim();
    const text = text1.replace(/[^\w\s]/gi, '').trim();
    if (!text) {
        return [];
    } else {
